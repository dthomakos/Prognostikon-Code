...
class CyberneticTrader:
    def __init__(self, data, initial_capital=100000,
                 ...
                 # Trading parameters
                 transaction_cost=0.0001, start_idx=50,
    ...
    def walk_forward_optimize(self, train_size=0.7, window_type='expanding', step_size=63,
                              param_grid=None, metric='sharpe_ratio'):
        """
        Perform walk-forward optimization for parameter tuning

        Parameters:
        train_size : float or int
            Proportion (0-1) or number of days for training
        window_type : str
            'expanding' or 'rolling' window
        step_size : int
            Days between optimizations (rebalance frequency)
        param_grid : dict
            Parameters to optimize with possible values
        metric : str
            Performance metric to optimize ('sharpe_ratio', 'return', 'drawdown')
        """
        # Prepare parameter grid
        param_grid = param_grid or {
            'learning_rate': [0.001, 0.005, 0.01],
            'effector_threshold': [0.05, 0.1, 0.15],
            'volterra_memory': [5, 10, 15],
            'weights': [
        ...
        # Convert train_size to number of days if proportion
        ...
        # Create date range for optimization
        dates = self.data.index[train_size:]
        date_ranges = [(dates[i], dates[min(i+step_size, len(dates)-1)])
                      for i in range(0, len(dates), step_size)]

        # Walk-forward optimization loop
        for start_date, end_date in tqdm(date_ranges, desc="WFO Progress"):
            # Update training window
            if window_type == 'expanding':
                train_data = self.data.loc[:start_date]
            else:  # rolling window
                train_start = self.data.index.get_loc(start_date) - train_size
                train_data = self.data.iloc[train_start:self.data.index.get_loc(start_date)]

            test_data = self.data.loc[start_date:end_date]

            # Find best parameters on training set
            best_params, best_score = self._optimize_params(train_data, param_grid, metric)
            best_params_history.append((start_date, best_params))

            # Test with best parameters
            trader = CyberneticTrader(test_data, initial_capital=self.capital, **best_params)
            portfolio, _ = trader.backtest()
            portfolio_history.extend(portfolio)
            ...
            results.append({
                ...
                'test_performance': portfolio[-1] / portfolio[0] - 1
            })

        # Update trader with best final parameters
        self._apply_best_params(best_params_history[-1][1])

        # Generate equity curve
        equity_curve = pd.Series(portfolio_history, index=self.data.index[-len(portfolio_history):])

        return results, equity_curve, best_params_history

    def _optimize_params(self, data, param_grid, metric='sharpe_ratio'):
        ...
        for params in tqdm(param_combinations, desc="Parameter Optimization", leave=False):
            try:
                # Initialize trader with current parameters
                trader = CyberneticTrader(data, initial_capital=100000, **params)
                ...
                if metric == 'sharpe_ratio':
                    score = returns.mean() / (returns.std() + 1e-8) * np.sqrt(252)
                elif metric == 'return':
                    score = portfolio[-1] / portfolio[0] - 1
                elif metric == 'calmar':
                    dd = self.calculate_max_drawdown(portfolio)
                    score = (portfolio[-1] / portfolio[0] - 1) / (abs(dd) + 1e-8)
                else:
                    score = returns.mean()
    ...
    def _apply_best_params(self, best_params):
        """Apply best parameters to current instance"""
        for key, value in best_params.items():
            if hasattr(self, key):
                setattr(self, key, value)

        # Reinitialize system with new parameters
        self.initialize_system()

    def calculate_max_drawdown(self, portfolio_values):
        """Calculate maximum drawdown for a portfolio series"""
...
if __name__ == "__main__":
    ...
    data = yf.download('SPY', start='2022-01-01', end='2025-06-30')
    ...
    trader = CyberneticTrader(
        ...
        # ... [other parameters] ... but we do WFO so no need here!!
    )

    # Perform Walk-Forward Optimization
    wfo_results, equity_curve, best_params_history = trader.walk_forward_optimize(
        train_size=0.6,  # 60% of data for initial training
        step_size=63,    # Quarterly reoptimization
        window_type='expanding',
        metric='sharpe_ratio',
        param_grid={
            'learning_rate': [0.005, 0.01],
            'effector_threshold': [0.05, 0.1],
            'volterra_memory': [2, 5],
            'weights': [
                np.array([0.5, 0.2, 0.2, 0.1]),
                np.array([0.4, 0.3, 0.2, 0.1]),
    ...
    equity_curve.plot(title='Walk-Forward Optimization Equity Curve')
    ...
    param_df = pd.DataFrame({
        date: params for date, params in best_params_history
    }).T

    plt.figure(figsize=(14, 10))
    for i, param in enumerate(param_df.columns, 1):
        plt.subplot(len(param_df.columns), 1, i)
        param_df[param].plot(title=f'{param} Evolution')
        plt.ylabel(param)
    plt.tight_layout()
    plt.show()

    # Generate performance report with optimized parameters
