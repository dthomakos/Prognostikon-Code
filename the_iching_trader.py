...
def generate_signals(price, window=20, risk_aversion=1, delay=1):
    ...
    # Prepare DataFrame to hold all info
...
# --- Backtesting ---
def backtest(price, signals_df, add_2delay=0, leverage=True, f_factor=252, signal_switch=1):
    ...
    if leverage:
        signals = np.sign(signals_df['action'])
    else:
        signals = signals_df['action']
...
strat_returns, cum_rets, cumr_bench, performance = backtest(price, signals_df, set_add_2delay, set_leverage, set_f_factor, set_signal_switch)

print("Performance Metrics:")
for k, v in performance.items():
    print(f"{k}: {v:.4f}")

# --- Show sample signals with meanings ---
print("\nSample signals with hexagram meanings:")
print(signals_df[['hexagram', 'hexagram_name', 'action', 'confidence', 'regime', 'element', 'comment']].tail(20))

# --- Plot ---
plt.figure(figsize=(12,6))
plt.plot(cum_rets, label='The I Ching Trader')
plt.plot(cumr_bench, label='Buy & Hold')
plt.legend()
plt.grid(which='both')
plt.xlabel('Time')
plt.ylabel('Return in decimal')
plt.title('I Ching Hexagram Trading Strategy vs Buy & Hold for '+ticker)
plt.show()
