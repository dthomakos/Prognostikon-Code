...
def momentum_func(returns):
    return np.prod(1 + returns) - 1

def volatility_func(returns):
    return np.std(returns)

def returns_func(returns):
    return np.mean(returns)

def trend_func(returns):
    return (1 + returns[-1]) / (1 + returns[0]) - 1
...
if __name__ == "__main__":
    ...
    # Initialize trader
    trader = CoxProbabilisticTrader_V1(
